...
La hipótesis de partida del presente trabajo sostiene que la implementación de un módulo analítico externo, desacoplado del sistema transaccional de Odoo, permitirá reducir drásticamente el tiempo que los responsables de proyecto y directores dedican a obtener información útil sobre el estado de sus proyectos. Actualmente, extraer indicadores relevantes requiere navegar manualmente por múltiples vistas del ERP, aplicar filtros sucesivos y cruzar datos de forma manual; un proceso lento, propenso a errores y que no escala cuando se necesitan comparaciones entre proyectos, equipos o periodos.

La solución propuesta reemplaza ese proceso por consultas automatizadas que, con un único clic, entregan al responsable análisis completos y comparaciones entre proyectos, recursos o periodos, sin necesidad de configurar filtros manuales ni exportar datos a hojas de cálculo. Esto se traduce en una mejora sustancial en la toma de decisiones estratégicas, al proporcionar información relevante, precisa y oportuna que permita anticipar riesgos, detectar desviaciones tempranas en coste y planificación, y optimizar la asignación de recursos.

El **objetivo general** del presente TFG es **disminuir significativamente el tiempo que el manager o responsable dedica a consultar el estado de los proyectos**, mediante un módulo externo orientado al análisis de datos del ERP que automatice la generación de métricas e indicadores clave de rendimiento (KPIs) y permita realizar consultas complejas y comparaciones entre proyectos, recursos o periodos con un solo clic, sin requerir configuración manual de filtros. Además, el módulo ofrecerá indicadores estratégicos no disponibles en el sistema transaccional, respondiendo a las necesidades específicas de los directores y responsables de proyecto de la empresa.
